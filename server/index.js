...
async function callEvaluatorWithRetry(url, payload, options = {}) {
  ...
  while (attempt <= maxRetries) {
    ...
    try {
      ...
      const isRetryable = isTimeout || isNetwork || isServer;
...
async function callEvaluatorWithFallback(payload) {
  const primaryEvaluator = process.env.PRIMARY_EVALUATOR || 'deepeval';
  const deepevalUrl = process.env.DEEPEVAL_URL || 'http://localhost:8000/eval';
  const ragasUrl = process.env.RAGAS_URL || 'http://localhost:8000/eval-ragas';

  const primaryUrl = primaryEvaluator === 'ragas' ? ragasUrl : deepevalUrl;
  const secondaryUrl = primaryEvaluator === 'ragas' ? deepevalUrl : ragasUrl;
  const secondaryName = primaryEvaluator === 'ragas' ? 'deepeval' : 'ragas';

  const primary = await callEvaluatorWithRetry(primaryUrl, payload);
  if (primary.success) {
    return { ...primary, evaluator: primaryEvaluator, fallbackUsed: false };
  }

  const secondary = await callEvaluatorWithRetry(secondaryUrl, payload);
  if (secondary.success) {
    return { ...secondary, evaluator: secondaryName, fallbackUsed: true };
  }

  return {
    success: false,
    error: secondary.error || primary.error,
    latencyMs: (primary.latencyMs || 0) + (secondary.latencyMs || 0),
    retries: (primary.retries || 0) + (secondary.retries || 0),
    errorClass: 'all_evaluators_unavailable',
    evaluator: 'none',
    fallbackUsed: true,
  };
}

// POST /api/evaluate: evaluator bridge with retry, fallback, and observability metadata
app.post('/api/evaluate', async (req, res) => {
  const startedAt = Date.now();
  const requestId =
    req.headers['x-request-id'] || `eval-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;

  try {
    const evalPayload = { ...(req.body || {}) };
    if (evalPayload.metric === 'faithfullness') {
      evalPayload.metric = 'faithfulness';
    }

    const result = await callEvaluatorWithFallback(evalPayload);
    ...
    if (result.success) {
      ...
      const normalizedResults = Array.isArray(raw.results)
        ? raw.results
        : Object.entries(raw.results || {}).map(([metricName, score]) => ({
            metric_name: metricName,
            score,
            explanation: raw.details?.[metricName] || null,
          }));
    ...
    const mappedStatus = result.errorClass === 'validation' ? 400 : 503;
    const message =
      result.error?.response?.data?.detail ||
      result.error?.response?.data?.error ||
      (result.errorClass === 'timeout' ? 'Evaluation timeout' : 'Evaluator unavailable');

    return res.status(mappedStatus).json({
      error: mappedStatus === 400 ? 'EvaluatorValidationError' : 'EvaluatorUnavailable',
      message,
      details: {
